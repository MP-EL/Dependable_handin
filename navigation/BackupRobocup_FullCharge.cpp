...
//       system("espeak \"press green to start\" -ven+f4 -s130 -a5 2>/dev/null &");
    ...
    case 10: // PART 1 - DROP BALL IN HOLE
      line = 0;
      snprintf(lines[line++], MAX_LEN, "servo=2, pservo=400, vservo=130");
      snprintf(lines[line++], MAX_LEN, "vel=0.2: dist=0.1");
      snprintf(lines[line++], MAX_LEN, "vel=0.7: dist=0.05, lv=20");
      snprintf(lines[line++], MAX_LEN, "vel=0.7, edger=1.0, white=1: ir1<0.2");
      snprintf(lines[line++], MAX_LEN, "vel=0.7, edger=0.0, white=1: dist=0.2");
      snprintf(lines[line++], MAX_LEN, "vel=0.2: dist=0.3, lv=20");
      snprintf(lines[line++], MAX_LEN, "vel=0.6, edger=0.0, white=1: ir1<0.20");
      snprintf(lines[line++], MAX_LEN, "vel=0.5, edger=0.0, white=1: dist=1.3");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.0: turn=90.0");
      snprintf(lines[line++], MAX_LEN, "vel=0.2: dist=0.17");
      snprintf(lines[line++], MAX_LEN, "vel=0.0: time=0.5");
      snprintf(lines[line++], MAX_LEN, "servo=2, pservo=-150, vservo=110:time=7");
      snprintf(lines[line++], MAX_LEN, "vel=-0.3: dist=0.16");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.0: turn=-100.0");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, edger=0, white=1: dist=0.2");
      snprintf(lines[line++], MAX_LEN, "vel=0.3: dist=0.66");
      snprintf(lines[line++], MAX_LEN, "vel=0.1, tr=0.0: turn=-50.0");
      snprintf(lines[line++], MAX_LEN, "vel=0.1, tr=0.0: turn=30");
      snprintf(lines[line++], MAX_LEN, "vel=0.1, tr=0.0: turn=-30");
      snprintf(lines[line++], MAX_LEN, "vel=0, event=1 : time=5");    
      sendAndActivateSnippet(lines, line);
      state = 11;
      break;

    case 11: // check if case is done
      printf("Case 11");
      // wait for event 1 (send when finished driving first part)
      if (bridge->event->isEventSet(1)){ // finished first drive
        printf("Case nextState = 12");
        state = 12;
      }
      break;

    case 12: // PART 2 - RAISE SERVO AND GO TO END OF VIPPE
      printf("Case 12");
      line = 0;
      snprintf(lines[line++], MAX_LEN, "vel=0, servo=2, pservo=400, vservo=110: time=6");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.0: turn=-110.0");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, edger=0.0: dist=1.3");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, edger=0.0, white=1: xl>15, dist=2");
      snprintf(lines[line++], MAX_LEN, "vel=0.2, edger=0, white=1: dist=0.05");
      snprintf(lines[line++], MAX_LEN, "vel=0.0: time=0.3");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.0: turn=-90");
      snprintf(lines[line++], MAX_LEN, "vel=0.2, edger=1.0, white=1: dist=0.25");
      snprintf(lines[line++], MAX_LEN, "vel=0.2, edger=1.0, white=1: dist=1.0");
      snprintf(lines[line++], MAX_LEN, "vel=0.0: time=0.5");
      snprintf(lines[line++], MAX_LEN, "servo=2, pservo=-150, vservo=200: time=3");
      snprintf(lines[line++], MAX_LEN, "vel=0.1, edger=2.0, white=1: LV=0, dist=1.5");
      snprintf(lines[line++], MAX_LEN, "vel=0.2: dist=0.35");
      snprintf(lines[line++], MAX_LEN, "vel=0, event=2: time=5");        
      sendAndActivateSnippet(lines, line);
      state = 13;
      break;

    case 13: // check if case is done
      printf("Case 13");
      // wait for event 1 (send when finished driving first part)
      if (bridge->event->isEventSet(2)){ // finished first drive
        printf("Case nextState = 14");
        state = 14;
      }
      break;

    case 14: // PART 3 - END OF VIPPE TO END OF OPADG�ENDE RAMPE
      printf("Case 14");
      line = 0;
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.0: turn=-30");
      snprintf(lines[line++], MAX_LEN, "vel=0.25: dist=1");
      //snprintf(lines[line++], MAX_LEN, "vel=0.25: LV=1,dist=3");
      snprintf(lines[line++], MAX_LEN, "vel=0.25: xl>15, dist=3");
      snprintf(lines[line++], MAX_LEN, "vel=0.0: time=0.3");
      snprintf(lines[line++], MAX_LEN, "vel=0.2, edger=0, white=1: dist=0.05");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.0: turn=-90");
      snprintf(lines[line++], MAX_LEN, "vel=0.25, edger=0, white=1:time=10");
      snprintf(lines[line++], MAX_LEN, "vel=-0.2: dist=0.2");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.0: turn=-180");
      snprintf(lines[line++], MAX_LEN, "vel=0.2, edger=0, white=1: dist=0.3");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.0: turn=-90");
      snprintf(lines[line++], MAX_LEN, "vel=0.3,white=1: dist=0.55");
      snprintf(lines[line++], MAX_LEN, "vel=0.2, white=1: dist=0.05");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.0: turn=-90"); 
      snprintf(lines[line++], MAX_LEN, "vel=0: time=1");        
      snprintf(lines[line++], MAX_LEN, "servo=2, pservo=-140, vservo=200: time=3");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, edger=0, white=1: ir1<0.20");
      snprintf(lines[line++], MAX_LEN, "vel=0: time=1");
      snprintf(lines[line++], MAX_LEN, "servo=2, pservo=2000, vservo=0: time=3");
      snprintf(lines[line++], MAX_LEN, "vel=0, event=3: time=5");
      sendAndActivateSnippet(lines, line);
      state = 15;
      break;

    case 15: // check if case is done
      printf("Case 15");
      // wait for event 1 (send when finished driving first part)
      if (bridge->event->isEventSet(3)){ // finished first drive
        printf("Case nextState = 16");
        state = 16;
      }
      break;

    case 16: // PART 1 - RAISE SERVO AND GO TO END OF VIPPE
      printf("Case 16");
      line = 0;
      snprintf(lines[line++], MAX_LEN, "vel=0.2, edger=0, white=1: dist=0.3");
      snprintf(lines[line++], MAX_LEN, "vel=0: time=1");
      ...
      snprintf(lines[line++], MAX_LEN, "vel=0.1, tr=0.0: turn=40.0");
      snprintf(lines[line++], MAX_LEN, "vel=0.1, tr=0.0: turn=-30");
      snprintf(lines[line++], MAX_LEN, "vel=0.1, tr=0.0: turn=30");
      snprintf(lines[line++], MAX_LEN, "servo=2, pservo=400, vservo=130");
      ...
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.0: turn=-90.0");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, edger=0.0: dist=1.3");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, edger=0.0, white=1: xl>15, dist=4");
      snprintf(lines[line++], MAX_LEN, "vel=0.2, edger=0, white=1: dist=0.05");
      snprintf(lines[line++], MAX_LEN, "vel=0.0: time=0.3");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.0: turn=-180"); 
      snprintf(lines[line++], MAX_LEN, "vel=0.3, edgel=0, white=1: dist=1.2");        
      snprintf(lines[line++], MAX_LEN, "vel=0.0: time=0.3");
      snprintf(lines[line++], MAX_LEN, "servo=2, pservo=-140, vservo=200: time=2");
      snprintf(lines[line++], MAX_LEN, "vel=0.0: time=0.3");
      snprintf(lines[line++], MAX_LEN, "vel=0, event=4: time=5");
      sendAndActivateSnippet(lines, line);
      state = 17;
      break;

    case 17: // check if case is done
      printf("Case 17");
      // wait for event 1 (send when finished driving first part)
      if (bridge->event->isEventSet(4)){ // finished first drive
        printf("Case nextState = 18");
        state = 18;
      }
      break;

    case 18: // PART 1 - RAISE SERVO AND GO TO END OF VIPPE
      printf("Case 18");
      line = 0;
      snprintf(lines[line++], MAX_LEN, "vel=0.35, edger=1, white=1: dist=2.2");
      snprintf(lines[line++], MAX_LEN, "vel=-0.2 : time = 3");
      snprintf(lines[line++], MAX_LEN, "vel=0.25, edger=0, white=1: dist=1.35");
      snprintf(lines[line++], MAX_LEN, "vel=0.3, tr=0.0: turn=-90.0");
      snprintf(lines[line++], MAX_LEN, "vel=0.3: dist=1.65");
      snprintf(lines[line++], MAX_LEN, "vel=0.25, edger=1, white=1: dist=0.50");
      snprintf(lines[line++], MAX_LEN, "vel=0, event=5: time=6");
      sendAndActivateSnippet(lines, line);
      state = 19;
      break;

    case 19: // check if case is done
      printf("Case 19");
      // wait for event 1 (send when finished driving first part)
      if (bridge->event->isEventSet(5)){ // finished first drive
        printf("Case nextState = 999");
        state = 999;
      }
      break;
...
//           system("espeak \"found marker.\" -ven+f4 -s130 -a5 2>/dev/null &"); 
    ...
    case 21: // wait until manoeuvre has finished
      ...
      {// repeat looking (until all 360 degrees areedgel=0, white=1: dist=0.5 tested)
        if (featureCnt < 36)
          state = 11;
        else
          state = 999;
